...
import { normalizeRelayUrl as normalizeRelayUrlUtil, RelayUrlValidationError, } from "../utils/relayUrl.js";
import { safeArrayAccess, SecurityValidationError, } from "../utils/security-validator.js";
 ...
 */
export function createRelayDiscoveryEvent(options, pubkey) {
 ...
 */
export function parseRelayDiscoveryEvent(event) {
 ...
 */
export function createRelayMonitorAnnouncement(options, pubkey) {
    // Validate required fields
    if (!options) {
        throw new Error("Options object is required");
    }
 ...
 */
export function parseRelayMonitorAnnouncement(event) {
    if (event.kind !== RELAY_MONITOR_KIND)
        return null;
    const data = {
        frequency: 0,
        timeouts: [],
