...
import { NIP46Error, NIP46ConnectionError, NIP46Method, } from "./types.js";
import { Logger, LogLevel } from "./utils/logger.js";
import { createSuccessResponse, createErrorResponse, } from "./utils/request-response.js";
import { buildConnectionString } from "./utils/connection.js";
import { validatePrivateKeySecure } from "./utils/security.js";
/**
...
export class SimpleNIP46Bunker {
    ...
    async handleRequest(event) {
        try {
            ...
            try {
                const decrypted = decryptNIP44(event.content, this.signerKeys.privateKey, event.pubkey);
                ...
                const response = createErrorResponse("unknown", // cannot recover id - using convention for failed parse
            ...
            try {
                ...
                const response = createErrorResponse("unknown", // cannot recover id
    ...
    async handleConnect(request, clientPubkey) {
        ...
        if (!request.params || request.params.length < 1) {
            return createErrorResponse(request.id, "Missing parameters for connect request");
        }
        // Extract parameters
        ...
        if (requestedSignerPubkey !== this.signerKeys.publicKey) {
            this.logger.warn(`Client requested connection to ${requestedSignerPubkey} but this bunker serves ${this.signerKeys.publicKey}`);
            return createErrorResponse(request.id, "Requested signer pubkey does not match this bunker");
        }
        ...
        if (permissionsParam && typeof permissionsParam === "string") {
            permissionsParam.split(",").forEach((permission) => {
        ...
        this.logger.debug(`Client permissions: ${Array.from(session.permissions).join(", ")}`);
    ...
    async handleSignEvent(request, clientPubkey) {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            return createErrorResponse(request.id, `Failed to sign event: ${errorMessage}`);
        }
    }
    /**
     * Handle NIP-44 encryption request (preferred)
     */
    async handleNIP44Encrypt(request, clientPubkey) {
        // Check authorization
        if (!this.isClientAuthorized(clientPubkey)) {
            return createErrorResponse(request.id, "Unauthorized");
        }
        ...
        if (!client.permissions.has("nip44_encrypt")) {
            return createErrorResponse(request.id, "Not authorized to encrypt NIP-44 messages");
        }
        try {
     ...
     */
    async handleNIP44Decrypt(request, clientPubkey) {
        // Check authorization
        if (!this.isClientAuthorized(clientPubkey)) {
            return createErrorResponse(request.id, "Unauthorized");
        }
        ...
        if (!client.permissions.has("nip44_decrypt")) {
            return createErrorResponse(request.id, "Not authorized to decrypt NIP-44 messages");
        }
        try {
    ...
    async handleNIP04Encrypt(request, clientPubkey) {
        ...
        if (!client.permissions.has("nip04_encrypt")) {
            return createErrorResponse(request.id, "Not authorized to encrypt NIP-04 messages");
        }
        try {
            const [thirdPartyPubkey, plaintext] = request.params;
    ...
    async handleNIP04Decrypt(request, clientPubkey) {
        ...
        if (!client.permissions.has("nip04_decrypt")) {
            return createErrorResponse(request.id, "Not authorized to decrypt NIP-04 messages");
        }
        try {
            const [thirdPartyPubkey, ciphertext] = request.params;
     ...
     */
    async handleGetRelays(request, clientPubkey) {
        // Check authorization
        if (!this.isClientAuthorized(clientPubkey)) {
            return createErrorResponse(request.id, "Unauthorized");
        }
        ...
        if (!client.permissions.has("get_relays")) {
            return createErrorResponse(request.id, "Not authorized to get relay list");
        }
        try {
    ...
    async sendResponse(response, clientPubkey) {
        try {
            this.logger.debug(`Sending response for request ${response.id}:`, JSON.stringify(response));
            // Encrypt the response with the signer's private key and client's public key
            const encrypted = encryptNIP44(JSON.stringify(response), this.signerKeys.privateKey, clientPubkey);
            // Create the unsigned event
            const eventData = {
                kind: 24133,
                pubkey: this.signerKeys.publicKey,
            ...
            const signedEvent = await createSignedEvent(eventData, this.signerKeys.privateKey);
