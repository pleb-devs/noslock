...
import { NIP46ConnectionError, NIP46SecurityError } from "../types.js";
import { validatePubkey, validateRelayUrl, validatePermission, sanitizeString, } from "./validator.js";
/**
...
export function parseConnectionString(str) {
    ...
    if (!str.startsWith("bunker://") && !str.startsWith("nostrconnect://")) {
        throw new NIP46ConnectionError("Invalid connection string format. Must start with bunker:// or nostrconnect://");
    }
    ...
    if (!validatePubkey(pubkey)) {
        throw new NIP46ConnectionError("Invalid signer public key in connection string");
    }
    try {
        ...
        if (allRelays.length > 0 && relays.length === 0) {
            throw new NIP46ConnectionError(`All relay URLs in connection string are invalid. Provided relays: ${allRelays.join(", ")}`);
        }
        ...
        if (error instanceof NIP46SecurityError ||
