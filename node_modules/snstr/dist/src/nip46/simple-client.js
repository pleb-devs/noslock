...
class SimpleNIP46Client {
    ...
    constructor(relays, options = {}) {
        ...
        const logLevel = options.logLevel || (this.debug ? logger_1.LogLevel.DEBUG : logger_1.LogLevel.INFO);
    ...
    async connect(connectionString) {
        try {
            ...
            const info = (0, connection_1.parseConnectionString)(connectionString);
            ...
            if (!this.signerPubkey) {
                throw new types_1.NIP46ConnectionError("Signer public key is not set. Connection string parsing may have failed.");
            }
            const connectParams = [
                this.signerPubkey,
            ...
            const connectResponse = await this.sendRequest(types_1.NIP46Method.CONNECT, connectParams);
            ...
            if (connectResponse.error) {
                throw new types_1.NIP46ConnectionError(`Connection failed: ${connectResponse.error}`);
            }
            if (connectResponse.result !== "ack") {
                ...
                if (!info.secret || info.secret !== connectResponse.result) {
                    throw new types_1.NIP46ConnectionError("Invalid or missing required secret");
                }
            }
            ...
            try {
                ...
                throw new types_1.NIP46ConnectionError("Failed to get user public key after connect");
            }
        }
        catch (error) {
            // Clean up on error
            ...
            if (error instanceof types_1.NIP46Error) {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                throw new types_1.NIP46ConnectionError(`Connection failed: ${errorMessage}`);
            }
        }
    }
    /**
    ...
    async ping() {
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new types_1.NIP46TimeoutError("Ping timed out")), this.timeout).unref(); // Don't keep process alive
            });
            const pingPromise = this.sendRequest(types_1.NIP46Method.PING, []).then((response) => response.result === "pong");
    ...
    async signEvent(eventData) {
        ...
        if (response.error) {
            throw new types_1.NIP46SigningError(response.error);
        }
        // Parse the result
    ...
    async sendRequest(method, params) {
        return new Promise((resolve, reject) => {
            ...
            if (!this.clientKeys.privateKey) {
                reject(new types_1.NIP46ConnectionError("Client private key not set"));
                return;
            }
            if (!this.signerPubkey) {
                reject(new types_1.NIP46ConnectionError("Signer public key not set"));
                return;
            }
            // Create the request
            const request = {
            ...
            const timeoutId = setTimeout(() => {
                ...
                reject(new types_1.NIP46TimeoutError(`Request timed out: ${method}`));
            ...
            this.pendingRequests.set(request.id, (response) => {
                ...
                if (response.error) {
                    reject(new types_1.NIP46Error(response.error));
                }
                else {
                    resolve(response);
                }
            ...
            try {
                const encrypted = (0, nip44_1.encrypt)(JSON.stringify(request), this.clientKeys.privateKey, this.signerPubkey);
                ...
                (0, event_1.createSignedEvent)(eventData, this.clientKeys.privateKey)
                    .then((signedEvent) => {
                    ...
                    if (!publishResult.success) {
                        ...
                        throw new types_1.NIP46ConnectionError(`Relay rejected event: ${reasonMessage}`);
                    }
                })
                    .catch((err) => {
                    clearTimeout(timeoutId);
                    ...
                    reject(new types_1.NIP46ConnectionError(`Failed to sign or publish event: ${errorMessage}`));
                });
            }
            catch (error) {
                clearTimeout(timeoutId);
                this.pendingRequests.delete(request.id);
                const errorMessage = error instanceof Error ? error.message : String(error);
                reject(new types_1.NIP46EncryptionError(`Failed to encrypt request: ${errorMessage}`));
            }
        });
    }
    /**
    ...
    handleResponse(event) {
        ...
        try {
            ...
            const decrypted = (0, nip44_1.decrypt)(event.content, this.clientKeys.privateKey, event.pubkey);
