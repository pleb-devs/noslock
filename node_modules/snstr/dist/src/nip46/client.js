...
class NostrRemoteSignerClient {
    constructor(options = {}) {
        ...
        this.logger = new logger_1.Logger({
            level: options.debug ? logger_1.LogLevel.DEBUG : logger_1.LogLevel.INFO,
            prefix: "NIP46-CLIENT",
    ...
    async setupSubscription() {
        ...
        if (!this.clientKeypair) {
            throw new types_1.NIP46ConnectionError("Client keypair not initialized");
        }
        const filter = {
            kinds: [24133],
    ...
    async cleanup() {
        ...
        this.pendingRequests.forEach((request) => {
            ...
            request.reject(new types_1.NIP46ConnectionError("Client disconnected"));
        });
    ...
    async connect(connectionString) {
        ...
        try {
            ...
            const connectionInfo = (0, connection_1.parseConnectionString)(connectionString);
            ...
            if (connectionInfo.relays?.length) {
                ...
                this.nostr = new nostr_1.Nostr(Array.from(new Set(allRelays)));
                await this.nostr.connectToRelays();
            ...
            const response = await this.sendRequest(types_1.NIP46Method.CONNECT, params);
            if (response.error) {
                throw new types_1.NIP46ConnectionError(`Connection failed: ${response.error}`);
            }
            ...
            await this.cleanup();
            if (error instanceof types_1.NIP46Error) {
                throw error;
            }
            const message = error instanceof Error ? error.message : String(error);
            throw new types_1.NIP46ConnectionError(`Failed to connect: ${message}`);
        }
    }
    /**
    ...
    async disconnect() {
        ...
        try {
            if (this.connected && this.signerPubkey) {
                ...
                await this.sendRequest(types_1.NIP46Method.DISCONNECT, []);
    ...
    async signEvent(eventData) {
        ...
        if (!this.connected) {
            throw new types_1.NIP46ConnectionError("Not connected to signer");
        }
        ...
        if (response.error) {
            ...
            throw new types_1.NIP46SigningError(`Event signing failed: ${response.error}`);
        }
    ...
    async getUserPublicKey() {
        ...
        if (response.error) {
            throw new types_1.NIP46ConnectionError(`Failed to get public key: ${response.error}`);
        }
    ...
    async ping() {
        ...
        if (response.error) {
            ...
            throw new types_1.NIP46Error(`Ping failed: ${response.error}`);
        }
    ...
    async nip44Encrypt(thirdPartyPubkey, plaintext) {
        ...
        const response = await this.sendRequest(types_1.NIP46Method.NIP44_ENCRYPT, [
            thirdPartyPubkey,
            plaintext,
        ]);
        if (response.error) {
            this.logger.error("NIP-44 encryption failed", { error: response.error });
            throw new types_1.NIP46EncryptionError(`NIP-44 encryption failed: ${response.error}`);
        }
    ...
    async nip44Decrypt(thirdPartyPubkey, ciphertext) {
        ...
        const response = await this.sendRequest(types_1.NIP46Method.NIP44_DECRYPT, [
            thirdPartyPubkey,
            ciphertext,
        ]);
        if (response.error) {
            this.logger.error("NIP-44 decryption failed", { error: response.error });
            throw new types_1.NIP46DecryptionError(`NIP-44 decryption failed: ${response.error}`);
        }
    ...
    async getRelays() {
        ...
        const response = await this.sendRequest(types_1.NIP46Method.GET_RELAYS, []);
        if (response.error) {
            this.logger.error("Get relays failed", { error: response.error });
            throw new types_1.NIP46Error(`Get relays failed: ${response.error}`);
        }
    ...
    async nip04Encrypt(thirdPartyPubkey, plaintext) {
        ...
        const response = await this.sendRequest(types_1.NIP46Method.NIP04_ENCRYPT, [
            thirdPartyPubkey,
            plaintext,
        ]);
        if (response.error) {
            this.logger.error("NIP-04 encryption failed", { error: response.error });
            throw new types_1.NIP46EncryptionError(`NIP-04 encryption failed: ${response.error}`);
        }
    ...
    async nip04Decrypt(thirdPartyPubkey, ciphertext) {
        ...
        const response = await this.sendRequest(types_1.NIP46Method.NIP04_DECRYPT, [
            thirdPartyPubkey,
            ciphertext,
        ]);
        if (response.error) {
            this.logger.error("NIP-04 decryption failed", { error: response.error });
            throw new types_1.NIP46DecryptionError(`NIP-04 decryption failed: ${response.error}`);
        }
    ...
    async sendRequest(method, params) {
        if (!this.connected && method !== types_1.NIP46Method.CONNECT) {
            throw new types_1.NIP46ConnectionError("Client is not connected");
        }
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(new types_1.NIP46TimeoutError(`Request ${id} timed out`));
    ...
    async sendEncryptedRequest(request) {
        if (!this.clientKeypair || !this.signerPubkey) {
            throw new types_1.NIP46ConnectionError("Client keypair or signer pubkey not set");
        }
        try {
            const requestJson = JSON.stringify(request);
            const encryptedContent = await (0, nip44_1.encrypt)(requestJson, this.clientKeypair.privateKey, this.signerPubkey);
            const requestEvent = await (0, event_1.createSignedEvent)({
                kind: 24133,
            ...
            throw new types_1.NIP46ConnectionError(`Failed to send request: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async decryptContent(content, authorPubkey) {
        ...
        try {
            ...
            const decrypted = await (0, nip44_1.decrypt)(content, this.clientKeypair.privateKey, authorPubkey);
            return {
                success: true,
                method: "nip44",
                data: decrypted,
    ...
    handleAuthChallenge(response) {
        ...
        // Validate auth URL
        if (!(0, auth_1.isValidAuthUrl)(response.auth_url, {
    ...
    handleAuthTimeout(requestId) {
        ...
        if (pendingRequest) {
            ...
            pendingRequest.reject(new types_1.NIP46TimeoutError("Auth challenge timed out"));
        }
    }
    /**
    ...
    generateRequestId() {
        ...
        return (0, request_response_1.generateRequestId)();
    }
    /**
    ...
    static generateConnectionString(clientPubkey, options = {}) {
        ...
        if (!clientPubkey || clientPubkey.trim() === "") {
            throw new types_1.NIP46ConnectionError("Client pubkey cannot be empty");
        }
        ...
        const secret = options.secret || (0, request_response_1.generateRequestId)();
