...
exports.NostrWalletConnectClient = exports.NIP47ClientError = void 0;
exports.parseNWCURL = parseNWCURL;
exports.generateNWCURL = generateNWCURL;
 ...
 */
function generateNWCURL(options) {
    if (!options.pubkey) {
...
class NIP47ClientError extends Error {
    constructor(message, code, data) {
        super(message);
        this.name = "NIP47ClientError";
        this.code = code;
        // Determine error category
        this.category = types_1.ERROR_CATEGORIES[code] || types_1.NIP47ErrorCategory.INTERNAL;
        // Add recovery hint if available
        this.recoveryHint = types_1.ERROR_RECOVERY_HINTS[code];
    ...
    static fromResponseError(error) {
        return new NIP47ClientError(error.message || "Unknown error", error.code || types_1.NIP47ErrorCode.INTERNAL_ERROR, error.data);
    }
    /**
     * Check if the error is a network-related error
     */
    isNetworkError() {
        return this.category === types_1.NIP47ErrorCategory.NETWORK;
    }
    /**
     * Check if the error is a timeout
     */
    isTimeoutError() {
        return this.category === types_1.NIP47ErrorCategory.TIMEOUT;
    }
    /**
     * Check if the error is authorization-related
     */
    isAuthorizationError() {
        return this.category === types_1.NIP47ErrorCategory.AUTHORIZATION;
    }
    /**
    ...
    isRetriable() {
        return (this.isNetworkError() ||
            this.isTimeoutError() ||
            this.code === types_1.NIP47ErrorCode.WALLET_UNAVAILABLE);
    }
    /**
...
exports.NIP47ClientError = NIP47ClientError;
/**
 ...
 */
class NostrWalletConnectClient {
    constructor(options) {
        ...
        this.clientPubkey = (0, crypto_1.getPublicKey)(this.clientPrivkey);
        ...
        this.preferredEncryption =
            options.preferredEncryption || types_1.NIP47EncryptionScheme.NIP44_V2;
    }
    /**
                ...
                if (this.pendingRequests.size > 0) {
                    this.pendingRequests.forEach((pendingRequest) => {
                        pendingRequest.resolve({
                            result_type: types_1.NIP47Method.GET_INFO, // Use a placeholder method
                            result: null,
                            error: {
                                code: types_1.NIP47ErrorCode.CONNECTION_ERROR,
                                message: "Client disconnected before receiving response",
                                category: types_1.NIP47ErrorCategory.NETWORK,
                            },
                        });
                    });
    ...
    setupSubscription() {
        ...
        const responseFilter = {
            kinds: [
                types_1.NIP47EventKind.RESPONSE,
                types_1.NIP47EventKind.NOTIFICATION,
                types_1.NIP47EventKind.NOTIFICATION_NIP44,
            ],
            authors: [this.pubkey],
            "#p": [this.clientPubkey],
        };
        ...
        const infoFilter = {
            kinds: [types_1.NIP47EventKind.INFO],
            authors: [this.pubkey],
        };
    ...
    async handleEvent(event) {
        ...
        if (event.kind === types_1.NIP47EventKind.RESPONSE) {
            console.log(`Processing as RESPONSE event (kind ${types_1.NIP47EventKind.RESPONSE})`);
            await this.handleResponse(event);
        }
        else if (event.kind === types_1.NIP47EventKind.NOTIFICATION ||
            event.kind === types_1.NIP47EventKind.NOTIFICATION_NIP44) {
            console.log(`Processing as NOTIFICATION event (kind ${event.kind})`);
            await this.handleNotification(event);
        }
        else if (event.kind === types_1.NIP47EventKind.INFO) {
            console.log(`Processing as INFO event (kind ${types_1.NIP47EventKind.INFO})`);
            this.handleInfoEvent(event);
        }
        else {
            console.log(`Unknown event kind: ${event.kind}, expected one of: ${types_1.NIP47EventKind.RESPONSE}, ${types_1.NIP47EventKind.NOTIFICATION}, ${types_1.NIP47EventKind.NOTIFICATION_NIP44}, ${types_1.NIP47EventKind.INFO}`);
        }
    }
    /**
    ...
    validateResponse(response) {
        ...
        if (!response || typeof response !== "object") {
            throw new NIP47ClientError("Invalid response: not an object", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        ...
        if (!resp.result_type || typeof resp.result_type !== "string") {
            throw new NIP47ClientError("Invalid response: missing or invalid result_type", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        // Verify result_type is a known NIP47Method
        if (!Object.values(types_1.NIP47Method).includes(resp.result_type)) {
            throw new NIP47ClientError(`Invalid response: unknown result_type '${resp.result_type}'`, types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        // Check that error field exists (can be null)
        if (!("error" in resp)) {
            throw new NIP47ClientError("Invalid response: missing error field", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        // If error is not null, validate its structure
        if (resp.error !== null) {
            if (typeof resp.error !== "object") {
                throw new NIP47ClientError("Invalid response: error field must be an object or null", types_1.NIP47ErrorCode.INVALID_REQUEST);
            }
            const error = resp.error;
            // Check error has code and message
            if (!error.code || typeof error.code !== "string") {
                throw new NIP47ClientError("Invalid response: error must have a code field", types_1.NIP47ErrorCode.INVALID_REQUEST);
            }
            if (!error.message || typeof error.message !== "string") {
                throw new NIP47ClientError("Invalid response: error must have a message field", types_1.NIP47ErrorCode.INVALID_REQUEST);
            }
            // When there's an error, result should be null
            if (resp.result !== null) {
                throw new NIP47ClientError("Invalid response: when error is present, result must be null", types_1.NIP47ErrorCode.INVALID_REQUEST);
            }
        }
        // If no error, result should be defined and not null
        if (resp.error === null &&
            (resp.result === null || resp.result === undefined)) {
            throw new NIP47ClientError("Invalid response: when error is null, result must be defined and not null", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        return response;
    }
    /**
        ...
        try {
            ...
            try {
                if (encryptionScheme === types_1.NIP47EncryptionScheme.NIP44_V2) {
                    decrypted = await (0, nip44_1.decrypt)(event.content, this.clientPrivkey, this.pubkey);
                }
                else {
                    decrypted = (0, nip04_1.decrypt)(this.clientPrivkey, this.pubkey, event.content);
                }
            }
            catch (decryptError) {
    ...
    async handleNotification(event) {
        try {
            ...
            if (event.kind === types_1.NIP47EventKind.NOTIFICATION_NIP44) {
                decrypted = await (0, nip44_1.decrypt)(event.content, this.clientPrivkey, this.pubkey);
            }
            else {
    ...
    handleInfoEvent(event) {
        try {
            ...
            console.log("Event kind:", event.kind, "Expected:", types_1.NIP47EventKind.INFO);
            ...
            if (encryptionTag && typeof encryptionTag[1] === "string") {
                ...
                this.supportedEncryption = schemes
                    .map((s) => s)
                    .filter((s) => Object.values(types_1.NIP47EncryptionScheme).includes(s));
            }
            else {
                // If no encryption tag, assume only NIP-04 is supported (backwards compatibility)
                this.supportedEncryption = [types_1.NIP47EncryptionScheme.NIP04];
            }
    ...
    chooseEncryptionScheme() {
        ...
        if (this.supportedEncryption.includes(types_1.NIP47EncryptionScheme.NIP44_V2)) {
            return types_1.NIP47EncryptionScheme.NIP44_V2;
        }
        // Fall back to NIP-04
        return types_1.NIP47EncryptionScheme.NIP04;
    }
    /**
    ...
    async sendRequest(request, expiration) {
        if (!this.initialized) {
            throw new NIP47ClientError("Client not initialized", types_1.NIP47ErrorCode.NOT_INITIALIZED);
        }
        ...
        const eventTemplate = {
            kind: types_1.NIP47EventKind.REQUEST,
            content: "",
            tags,
        };
        // Create the event
        const event = (0, event_1.createEvent)(eventTemplate, this.clientPubkey);
        ...
        try {
            if (encryptionScheme === types_1.NIP47EncryptionScheme.NIP44_V2) {
                encryptedContent = await (0, nip44_1.encrypt)(JSON.stringify(request), this.clientPrivkey, this.pubkey);
            }
            else {
                encryptedContent = (0, nip04_1.encrypt)(this.clientPrivkey, this.pubkey, JSON.stringify(request));
            }
        }
        catch (error) {
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Failed to encrypt request: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.ENCRYPTION_ERROR, { originalError: error, encryptionScheme });
        }
        event.content = encryptedContent;
        // Generate event ID
        let eventId;
        try {
            eventId = await (0, event_1.getEventHash)(event);
        }
        catch (error) {
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Failed to generate event hash: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.INTERNAL_ERROR, { originalError: error });
        }
        ...
        const responsePromise = new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                ...
                reject(new NIP47ClientError("Request timed out", types_1.NIP47ErrorCode.TIMEOUT));
            }, timeoutMs);
        ...
        try {
            ...
            const signedEvent = await (0, event_1.createSignedEvent)(event, this.clientPrivkey);
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Failed to publish request: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.PUBLISH_FAILED, { originalError: error });
        }
        // Wait for response
    ...
    async getInfo(options) {
        try {
            const request = {
                method: types_1.NIP47Method.GET_INFO,
                params: {},
            };
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Error getting wallet info: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.INTERNAL_ERROR, { originalError: error });
        }
    }
    /**
    ...
    async withRetry(operation, retryOptions = {}) {
        ...
        throw new NIP47ClientError("Retry operation failed for an unknown reason", types_1.NIP47ErrorCode.INTERNAL_ERROR);
    }
    ...
    async getBalance(options) {
        const response = await this.sendRequest({
            method: types_1.NIP47Method.GET_BALANCE,
            params: {},
        }, options?.expiration);
        return response.result;
    }
    ...
    async payInvoice(invoice, amount, maxfee, options) {
        if (!invoice) {
            throw new NIP47ClientError("Invoice is required", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        try {
            const request = {
                method: types_1.NIP47Method.PAY_INVOICE,
                params: {
                    invoice,
                    amount,
                    maxfee,
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Error paying invoice: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.PAYMENT_FAILED, { originalError: error });
        }
    }
    /**
     * Make an invoice
    ...
    async makeInvoice(amount, description, descriptionHash, expiry, options) {
        ...
        if (amount === null || amount === undefined) {
            throw new NIP47ClientError("Amount is required", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        if (!description && !descriptionHash) {
            throw new NIP47ClientError("Description or description hash is required", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        try {
            const request = {
                method: types_1.NIP47Method.MAKE_INVOICE,
                params: {
                    amount,
                    description,
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Error creating invoice: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.INTERNAL_ERROR, { originalError: error });
        }
    }
    /**
    ...
    async lookupInvoice(params, options) {
        ...
        if (!params.payment_hash && !params.invoice) {
            throw new NIP47ClientError("Payment hash or invoice is required", types_1.NIP47ErrorCode.INVALID_REQUEST);
        }
        try {
            const request = {
                method: types_1.NIP47Method.LOOKUP_INVOICE,
                params: {
            ...
            if (error instanceof NIP47ClientError) {
                ...
                if (error.code === types_1.NIP47ErrorCode.NOT_FOUND) {
                    ...
                    throw new NIP47ClientError(`Invoice not found: Could not find ${lookupType}: ${lookupValue} in the wallet's database`, types_1.NIP47ErrorCode.NOT_FOUND);
                }
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            throw new NIP47ClientError(`Error looking up invoice: ${err.message || "Unknown error"}`, types_1.NIP47ErrorCode.LOOKUP_INVOICE_FAILED, { originalError: error });
        }
    }
    /**
     * List transactions
    ...
    async listTransactions(params = {}, options) {
        const response = await this.sendRequest({
            method: types_1.NIP47Method.LIST_TRANSACTIONS,
            params,
        }, options?.expiration);
    ...
    async signMessage(message, options) {
        const response = await this.sendRequest({
            method: types_1.NIP47Method.SIGN_MESSAGE,
            params: {
                message,
            },
        }, options?.expiration);
...
exports.NostrWalletConnectClient = NostrWalletConnectClient;
