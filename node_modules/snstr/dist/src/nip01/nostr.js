...
class Nostr {
    ...
    preprocessRelayUrl(url) {
        ...
        return (0, relayUrl_1.preprocessRelayUrl)(url);
    }
    ...
    addRelay(url) {
        ...
        if (!(0, nip19_1.isValidRelayUrl)(url)) {
            throw new Error(`Invalid relay URL: ${url}`);
        }
        if (this.relays.has(url)) {
            return this.relays.get(url);
        }
        const relay = new relay_1.Relay(url, this.relayOptions);
    ...
    async publishTextNote(content, tags = [], options) {
        // Rate limiting check
        const rateLimitCheck = (0, security_validator_1.checkRateLimit)(this.publishRateLimit, this.RATE_LIMITS.PUBLISH.limit, this.RATE_LIMITS.PUBLISH.windowMs);
        if (!rateLimitCheck.allowed) {
            throw new security_validator_1.SecurityValidationError(`Publish rate limit exceeded. Try again in ${Math.ceil((rateLimitCheck.retryAfter || 0) / 1000)} seconds`, "RATE_LIMIT_EXCEEDED", "publish");
        }
        ...
        // Validate inputs
        try {
            const validatedContent = (0, security_validator_1.validateEventContent)(content);
            const validatedTags = (0, security_validator_1.validateTags)(tags);
            ...
            if (contentByteLength > security_validator_1.SECURITY_LIMITS.MAX_CONTENT_SIZE) {
                throw new Error(`Content too large: ${contentByteLength} bytes (max ${security_validator_1.SECURITY_LIMITS.MAX_CONTENT_SIZE})`);
            }
            const noteTemplate = (0, event_1.createTextNote)(validatedContent, this.privateKey, validatedTags);
    ...
    async publishDirectMessage(content, recipientPubkey, tags = [], options) {
        ...
        const dmTemplate = await (0, event_1.createDirectMessage)(content, recipientPubkey, this.privateKey, tags);
    ...
    subscribe(filters, onEvent, onEOSE, options = {}) {
        // Rate limiting check
        const rateLimitCheck = (0, security_validator_1.checkRateLimit)(this.subscribeRateLimit, this.RATE_LIMITS.SUBSCRIBE.limit, this.RATE_LIMITS.SUBSCRIBE.windowMs);
        if (!rateLimitCheck.allowed) {
            throw new security_validator_1.SecurityValidationError(`Subscription rate limit exceeded. Try again in ${Math.ceil((rateLimitCheck.retryAfter || 0) / 1000)} seconds`, "RATE_LIMIT_EXCEEDED", "subscribe");
        }
        // Update rate limit state (increment count)
        this.subscribeRateLimit.count++;
        // Validate filters before sending to relays
        try {
            const validatedFilters = (0, security_validator_1.validateFilters)(filters);
    ...
    async fetchMany(filters, options = {}) {
        // Rate limiting check
        const rateLimitCheck = (0, security_validator_1.checkRateLimit)(this.fetchRateLimit, this.RATE_LIMITS.FETCH.limit, this.RATE_LIMITS.FETCH.windowMs);
        if (!rateLimitCheck.allowed) {
            throw new security_validator_1.SecurityValidationError(`Fetch rate limit exceeded. Try again in ${Math.ceil((rateLimitCheck.retryAfter || 0) / 1000)} seconds`, "RATE_LIMIT_EXCEEDED", "fetch");
        }
        // Update rate limit state (increment count)
        ...
        try {
            ...
            const maxWait = options.maxWait
                ? (0, security_validator_1.validateNumber)(options.maxWait, 0, 300000, "fetchMany.maxWait")
                : 5000;
