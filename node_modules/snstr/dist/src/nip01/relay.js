...
const inMemoryWebSocket_1 = require("../utils/inMemoryWebSocket");
const nostr_1 = require("../types/nostr");
const event_1 = require("./event");
const time_1 = require("../utils/time");
const crypto_1 = require("../utils/crypto");
const logger_1 = require("../nip46/utils/logger");
...
class Relay {
    constructor(url, options = {}) {
        ...
        this.maxEventBuffers = security_validator_1.SECURITY_LIMITS.MAX_RELAY_EVENT_BUFFERS;
        this.maxEventsPerBuffer = security_validator_1.SECURITY_LIMITS.MAX_EVENTS_PER_BUFFER;
        ...
        this.maxReplaceableEventPubkeys = security_validator_1.SECURITY_LIMITS.MAX_REPLACEABLE_EVENT_PUBKEYS;
        this.maxReplaceableEventsPerPubkey = security_validator_1.SECURITY_LIMITS.MAX_REPLACEABLE_EVENTS_PER_PUBKEY;
        this.addressableEvents = new Map();
        this.addressableEventAccessTimes = new Map(); // For LRU eviction
        this.maxAddressableEvents = security_validator_1.SECURITY_LIMITS.MAX_ADDRESSABLE_EVENTS;
        this.pendingValidationCounts = new Map();
        this.pendingEoseSubscriptions = new Set();
        this.url = url;
        this.logger = new logger_1.Logger({
            prefix: `Relay(${url})`,
            level: logger_1.LogLevel.WARN, // Default to WARN level for production use
    ...
    async connect() {
        ...
        const connectionPromise = new Promise((resolve, reject) => {
            try {
                const inMemorySocket = (0, inMemoryWebSocket_1.createInMemoryWebSocket)(this.url);
                if (inMemorySocket) {
                    this.ws = inMemorySocket;
                }
                else {
                    const WS = (0, websocket_1.getWebSocketImplementation)();
                ...
                const timeoutId = setTimeout(() => {
                    ...
                    if (this.ws && !this.connected) {
                        this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error("connection timeout"));
                        try {
                            this.ws.close();
                        }
                        catch (e) {
    ...
    scheduleReconnect() {
        ...
        let secureJitter;
        try {
            secureJitter = (0, security_validator_1.getSecureRandom)();
        }
        catch (error) {
    ...
    async publish(event, options = {}) {
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                const cleanup = () => {
                    this.off(nostr_1.RelayEvent.OK, handleOk);
                    this.off(nostr_1.RelayEvent.Error, handleError);
                    this.off(nostr_1.RelayEvent.Disconnect, handleDisconnect);
                    clearTimeout(timeoutId);
                };
                // Register the event handlers
                this.on(nostr_1.RelayEvent.OK, handleOk);
                this.on(nostr_1.RelayEvent.Error, handleError);
                this.on(nostr_1.RelayEvent.Disconnect, handleDisconnect);
    ...
    subscribe(filters, onEvent, onEOSE, options = {}) {
        ...
        const id = (0, security_validator_1.secureRandomHex)(16);
                ...
                if (Array.isArray(values)) {
                    for (const item of values) {
                        if (!this._isValidNip01FilterIdentifier(item)) {
                            ...
                            throw new event_1.NostrValidationError(`Invalid NIP-01 filter value for '${fieldName}': item '${item}' is not a 64-character lowercase hex string.`, fieldName, filter);
                        }
                    }
                }
            }
        ...
        switch (type) {
            case "EVENT": {
                ...
                if (typeof subscriptionId !== "string") {
                    this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error("Invalid subscription identifier in EVENT message"));
                    break;
                }
                ...
                if (!this.isNostrEvent(event)) {
                    ...
                    this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error(`Invalid event structure: ${JSON.stringify(event)}`));
                    break;
                }
                // Perform initial synchronous validation
                if (!this.performBasicValidation(event)) {
                    this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error(`Invalid event: ${event.id}`));
                    this.decrementPendingValidation(subscriptionId);
                    break;
                }
                ...
                this.validateEventAsync(event)
                    ...
                    if (isValid) {
                        ...
                        this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error(`Async validation failed for event: ${event.id}`));
                    }
                })
                    .catch((error) => {
                    this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error(`Validation error: ${error instanceof Error ? error.message : "unknown error"}`));
                })
                    .finally(() => {
                    this.decrementPendingValidation(subscriptionId);
                });
            ...
            case "NOTICE": {
                ...
                this.triggerEvent(nostr_1.RelayEvent.Notice, this.url, noticeStr);
                break;
            }
            case "OK": {
                ...
                this.triggerEvent(nostr_1.RelayEvent.OK, eventIdStr, successBool, parsedDetails);
                break;
            }
            case "CLOSED": {
                ...
                this.triggerEvent(nostr_1.RelayEvent.Closed, subIdStr, messageStr);
            ...
            case "AUTH": {
                ...
                if (this.isNostrEvent(challengeEvent)) {
                    this.triggerEvent(nostr_1.RelayEvent.Auth, challengeEvent);
                }
                else {
                    // If it's not a proper event, log an error
                    this.triggerEvent(nostr_1.RelayEvent.Error, this.url, new Error(`Invalid AUTH challenge: ${JSON.stringify(challengeEvent)}`));
                }
                break;
            }
            default:
    ...
    async validateEventAsync(event) {
        try {
            ...
            const calculatedId = await (0, event_1.getEventHash)(eventData);
            if (calculatedId !== event.id) {
                return false;
            }
            // Step 2: Validate signature
            return await (0, crypto_1.verifySignature)(event.id, event.sig, event.pubkey);
        }
        catch (error) {
    ...
    parseOkMessage(rawMessage) {
        ...
        if (rawMessage) {
            for (const nipPrefix of Object.values(nostr_1.NIP20Prefix)) {
